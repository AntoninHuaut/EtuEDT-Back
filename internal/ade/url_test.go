...
func TestGetAcademicYearDates_BeforeJuly(t *testing.T) {
	now := time.Date(2025, time.June, 15, 12, 0, 0, 0, time.UTC)
	first, last := GetAcademicYearDates(now, 7)

	wantFirst := time.Date(2024, time.July, 1, 0, 0, 0, 0, time.UTC)
	wantLast := time.Date(2025, time.June, 30, 0, 0, 0, 0, time.UTC)
...
func TestGetAcademicYearDates_InJuly(t *testing.T) {
	now := time.Date(2025, time.July, 1, 0, 0, 0, 0, time.UTC)
	first, last := GetAcademicYearDates(now, 7)

	wantFirst := time.Date(2025, time.July, 1, 0, 0, 0, 0, time.UTC)
	wantLast := time.Date(2026, time.June, 30, 0, 0, 0, 0, time.UTC)
...
func TestGetAcademicYearDates_AfterJuly(t *testing.T) {
...
func TestBuildURL_ContainsAllQueryParams(t *testing.T) {
	...
	firstDate := time.Date(2025, time.July, 1, 0, 0, 0, 0, time.UTC)
	lastDate := time.Date(2026, time.June, 30, 0, 0, 0, 0, time.UTC)
	...
	if got := q.Get("firstDate"); got != "2025-07-01" {
		t.Errorf("firstDate: got %q, want %q", got, "2025-07-01")
	}
	if got := q.Get("lastDate"); got != "2026-06-30" {
		t.Errorf("lastDate: got %q, want %q", got, "2026-06-30")
	}
}
